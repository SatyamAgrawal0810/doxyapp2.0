...
Future<void> _scheduleHabitDailyNotif({required int id, required String habitName, required int hour, required int minute}) async {
  ...
  try {
    ...
    await _notifs.zonedSchedule(id, 'Habit Reminder', 'Time to do: $habitName',
      scheduled,
      NotificationDetails(android: AndroidNotificationDetails('doxys_voice_reminders', 'Doxy Voice Reminders',
          importance: Importance.max, priority: Priority.high, playSound: true, enableVibration: true,
          styleInformation: BigTextStyleInformation('Time to do: $habitName', contentTitle: 'Habit Reminder'))),
      payload: 'Habit reminder. Time to do: $habitName',
      androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle,
      matchDateTimeComponents: DateTimeComponents.time,
      uiLocalNotificationDateInterpretation: UILocalNotificationDateInterpretation.absoluteTime);
  } catch (e) { debugPrint('Habit schedule error: $e'); }
}

Future<void> _notifyCheckin(String habitName, int streak) async {
  await _initHabitSystem();
  final msg = streak > 1 ? '$habitName done! $streak day streak!' : '$habitName done for today!';
  await _notifs.show(DateTime.now().millisecondsSinceEpoch ~/ 1000, 'Habit Complete',
...
Future<void> _cancelHabitNotif(int id) async { await _initHabitSystem(); await _notifs.cancel(id); }
...
class _HabitsScreenState extends State<HabitsScreen> {
  ...
  @override
  void initState() { super.initState(); _initHabitSystem(); _load(); }
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() => _habits = (body['data'] as List? ?? []).cast<Map<String, dynamic>>());
      }
  ...
  Future<void> _checkin(Map h) async {
    ...
    try {
      final res = await http.post(Uri.parse('$_base/$id/checkin'), headers: _headers,
          body: jsonEncode({})).timeout(const Duration(seconds: 10));
      if (!mounted) return;
      final body = jsonDecode(res.body);
      if (res.statusCode == 200) {
        final newStreak = body['streak']?['current'] ?? (h['streak'] as Map?)?['current'] ?? 0;
        await _notifyCheckin(name, newStreak);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(body['message'] ?? 'Checked in!${newStreak > 1 ? " $newStreak day streak!" : ""}'),
  ...
  void _showCreateSheet({Map<String, dynamic>? existing}) {
    showModalBottomSheet(context: context, isScrollControlled: true,
        ...
        builder: (_) => _HabitFormSheet(token: _token, existing: existing, onSaved: _load));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [IconButton(icon: const Icon(Icons.refresh, color: Color(0xFF8A8AAD)), onPressed: _load)],
      ),
      ...
      body: _loading
          ...
          : _habits.isEmpty ? _buildEmpty()
          : RefreshIndicator(onRefresh: _load, color: const Color(0xFF2979FF), child: _buildList()),
    );
  }

  Widget _buildEmpty() => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    Icon(Icons.track_changes, size: 64, color: const Color(0xFF1E1E38)),
    const SizedBox(height: 16),
    const Text('No habits yet', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
    const SizedBox(height: 8),
    const Text('Tap + to create your first habit', style: TextStyle(color: Color(0xFF8A8AAD), fontSize: 14)),
  ]));
  ...
  Widget _buildList() {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      Row(children: [
        ...
        const SizedBox(width: 10),
        _statCard('Streaks', '${_habits.where((h) => ((h['streak'] as Map?)?['current'] ?? 0) > 0).length}',
            Icons.local_fire_department, Colors.orange),
      ]),
      const SizedBox(height: 16),
      ..._habits.map((h) => _HabitCard(habit: h, isChecked: _isCheckedToday(h),
          onCheckin: () => _checkin(h), onEdit: () => _showCreateSheet(existing: h),
          onDelete: () => _delete(h['_id'] ?? h['id'] ?? '', h['name'] ?? ''))),
    ]);
  }
...
class _HabitCard extends StatelessWidget {
  ...
  const _HabitCard({required this.habit, required this.isChecked,
      required this.onCheckin, required this.onEdit, required this.onDelete});

  static const _categoryEmoji = {'health':'💚','fitness':'💪','mindfulness':'🧘',
      'productivity':'⚡','learning':'📚','social':'👥','custom':'✨'};
  ...
  Widget build(BuildContext context) {
    ...
    final remH = hasReminder ? ((reminders.first['time'] as Map?)?['hour'] ?? 9) : null;
    final remM = hasReminder ? ((reminders.first['time'] as Map?)?['minute'] ?? 0) : null;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            child: Center(child: Text(habit['icon'] ?? emoji,
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              Text(category, style: const TextStyle(color: Color(0xFF8A8AAD), fontSize: 12)),
              if (hasReminder && remH != null) ...[
                ...
                Container(
                  ...
                  child: Text(
                    ...
                    style: const TextStyle(color: Color(0xFF2979FF), fontSize: 10)),
                ),
              ],
            ]),
          ])),
          ...
          PopupMenuButton<String>(
            ...
            icon: const Icon(Icons.more_vert, color: Color(0xFF8A8AAD)),
        ...
        SizedBox(width: double.infinity,
          child: ElevatedButton.icon(
            ...
            icon: Icon(isChecked ? Icons.check_circle : Icons.circle_outlined, size: 18),
            label: Text(isChecked ? 'Completed Today' : 'Check In'),
            style: ElevatedButton.styleFrom(
                backgroundColor: isChecked ? Colors.green[800] : const Color(0xFF2979FF),
...
class _HabitFormSheet extends StatefulWidget {
  ...
  const _HabitFormSheet({required this.token, this.existing, required this.onSaved});
  @override
...
class _HabitFormSheetState extends State<_HabitFormSheet> {
  ...
  static const _categories = ['health','fitness','mindfulness','productivity','learning','social','custom'];
  static const _categoryEmoji = {'health':'💚','fitness':'💪','mindfulness':'🧘',
      'productivity':'⚡','learning':'📚','social':'👥','custom':'✨'};
  static const _icons = ['🎯','💪','🧘','📚','✨','🔥','💚','⚡','🌟','🎨','🏃','💼'];
  ...
  @override
  void dispose() { _nameCtl.dispose(); _descCtl.dispose(); super.dispose(); }
    ...
    final t = await showTimePicker(context: context,
      ...
      builder: (ctx, child) => Theme(
          data: Theme.of(ctx).copyWith(
              colorScheme: const ColorScheme.dark(primary: Color(0xFF2979FF))),
          child: child!));
    if (t != null) setState(() { _reminderHour = t.hour; _reminderMinute = t.minute; });
  }

  Future<void> _save() async {
    if (_nameCtl.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Please enter habit name'))); return;
    }
    setState(() => _saving = true);
    final payload = {
      'name': _nameCtl.text.trim(), 'description': _descCtl.text.trim(),
      'category': _category, 'icon': _icon,
      'goal': {'frequency': _frequency, 'targetCount': 1, 'unit': 'times'},
      'reminders': [{'enabled': _reminderEnabled, 'voiceEnabled': true,
    ...
    try {
      ...
      final id     = isEdit ? (widget.existing!['_id'] ?? widget.existing!['id'] ?? '') : '';
      final res    = isEdit
      ...
      if (res.statusCode == 200 || res.statusCode == 201) {
        ...
        final savedId  = resBody['data']?['_id'] ?? resBody['habit']?['_id'] ?? id;
        final notifId  = _habitNotifId(savedId);
        await _cancelHabitNotif(notifId);
        if (_reminderEnabled && savedId.isNotEmpty) {
          await _scheduleHabitDailyNotif(id: notifId, habitName: _nameCtl.text.trim(),
              hour: _reminderHour, minute: _reminderMinute);
        }
        Navigator.pop(context);
        widget.onSaved();
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text('${isEdit ? "Habit updated" : "Habit created"}${_reminderEnabled ? " · Reminder set ${_reminderHour.toString().padLeft(2,'0')}:${_reminderMinute.toString().padLeft(2,'0')}" : ""}'),
            backgroundColor: Colors.green[800]));
      } else {
        ...
        child: Column(children: [
          Padding(padding: const EdgeInsets.symmetric(vertical: 12),
              child: Container(width: 36, height: 4, decoration: BoxDecoration(
                  color: const Color(0xFF1E1E38), borderRadius: BorderRadius.circular(2)))),
          Padding(padding: const EdgeInsets.symmetric(horizontal: 20),
              child: Row(children: [
                Text(widget.existing != null ? 'Edit Habit' : 'New Habit',
                    style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                const Spacer(),
                IconButton(icon: const Icon(Icons.close, color: Color(0xFF8A8AAD)),
          ...
          Expanded(child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 12),
              _lbl('Description'),
              _fld(controller: _descCtl, hint: 'What do you want to achieve?', maxLines: 2),
              const SizedBox(height: 12),
              _lbl('Icon'),
              Wrap(spacing: 8, runSpacing: 8,
                  children: _icons.map((ic) => GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                          color: _icon == ic ? const Color(0xFF2979FF).withOpacity(0.15) : const Color(0xFF141428),
                          borderRadius: BorderRadius.circular(8),
                          border: Border.all(color: _icon == ic ? const Color(0xFF2979FF) : Colors.transparent)),
              ...
              Wrap(spacing: 8, runSpacing: 8,
                  children: _categories.map((cat) => GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                          color: _category == cat ? const Color(0xFF2979FF).withOpacity(0.15) : const Color(0xFF141428),
                          borderRadius: BorderRadius.circular(10),
                          border: Border.all(color: _category == cat ? const Color(0xFF2979FF) : Colors.transparent)),
                      child: Text('${_categoryEmoji[cat]} ${cat[0].toUpperCase()}${cat.substring(1)}',
                          style: TextStyle(
                              color: _category == cat ? const Color(0xFF2979FF) : Colors.white70,
                              fontSize: 12, fontWeight: FontWeight.w600))),
                  )).toList()),
              const SizedBox(height: 12),
              _lbl('Frequency'),
              Row(children: ['daily','weekly','custom'].map((f) => Expanded(child: Padding(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                        color: _frequency == f ? const Color(0xFF2979FF) : const Color(0xFF141428),
                        borderRadius: BorderRadius.circular(10)),
                    child: Text(f[0].toUpperCase() + f.substring(1), textAlign: TextAlign.center,
                        style: TextStyle(
                            color: _frequency == f ? Colors.white : const Color(0xFF8A8AAD),
                            fontSize: 12, fontWeight: FontWeight.w600))),
                )))).toList()),
              const SizedBox(height: 16),
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    const Icon(Icons.notifications_outlined, color: Color(0xFF2979FF), size: 18),
                    const SizedBox(width: 8),
                    const Text('Daily Reminder', style: TextStyle(
                  ...
                  if (_reminderEnabled) ...[
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Row(children: [
                          const Icon(Icons.access_time, color: Color(0xFF2979FF), size: 18),
                          const SizedBox(width: 10),
                          const Text('Reminder Time', style: TextStyle(color: Colors.white70, fontSize: 13)),
                          const Spacer(),
                          Container(
                            ...
                            child: Text(
                                ...
                                style: const TextStyle(
                                    color: Color(0xFF2979FF), fontSize: 16, fontWeight: FontWeight.bold))),
                          const SizedBox(width: 6),
                          const Icon(Icons.edit, color: Color(0xFF2979FF), size: 14),
                        ]),
                      ),
                    ),
                  ],
              ...
              child: SizedBox(width: double.infinity,
                child: ElevatedButton(
                  ...
                  child: _saving
                      ...
                      : Text(widget.existing != null ? 'Update Habit' : 'Create Habit',
  ...
  Widget _fld({required TextEditingController controller, required String hint, int maxLines = 1}) =>
      TextField(controller: controller, maxLines: maxLines,
        style: const TextStyle(color: Colors.white, fontSize: 14),
        decoration: InputDecoration(
            hintText: hint, hintStyle: const TextStyle(color: Color(0xFF4A4A6A), fontSize: 13),
