...
class _EnhancedCalendarScreenState extends State<EnhancedCalendarScreen> {
  ...
  Future<void> _loadAll() async {
    ...
    await Future.wait(
        [_loadEvents(), _loadReminders(), _loadHabits(), _loadRoutines()]);
  ...
  Future<void> _loadEvents() async {
    ...
    final cs = CalendarService(
        baseUrl: 'https://doxy-bh96.onrender.com/api', auth: auth);
    try {
      final data = await cs.getEvents();
      setState(() {
        _events.clear();
        for (final e in data) {
          // ── Skip routine-generated events — time zone wrong from server ──
          final eTitle = e['title'] as String? ?? '';
          final eDesc = e['description'] as String? ?? '';
          if (eTitle.contains('Morning') ||
              eTitle.contains('Evening') ||
              eDesc.contains('Morning Activities') ||
              eDesc.contains('Evening Activities') ||
              eDesc.contains('morning') ||
              eDesc.contains('evening')) {
            continue;
          }
  ...
  Future<void> _loadHabits() async {
    ...
    try {
      ...
      final habits = (jsonDecode(res.body)['data'] as List? ?? [])
          .cast<Map<String, dynamic>>();
      setState(() {
        _habitMarkers.clear();
        for (final h in habits) {
          final reminders = h['reminders'] as List? ?? [];
          // Show ALL habits on calendar — with reminder time if set, else 09:00
          int hour = 9, minute = 0;
  ...
  Future<void> _loadRoutines() async {
    ...
    try {
      ...
      final routines = ((body['data'] ?? body['routines'] ?? []) as List)
          .cast<Map<String, dynamic>>();
      setState(() {
        _routineMarkers.clear();
        for (final r in routines) {
          final scheduleDays =
              ((r['schedule'] as Map?)?['days'] as List?)?.cast<String>() ?? [];
          ...
          for (int i = 0; i < 30; i++) {
            ...
            if (morning['enabled'] == true) {
              ...
              // Morning activities — use actual stored time (local, not UTC)
  ...
  Future<void> _createEventForSelected(Map<String, dynamic> eventData) async {
    ...
    if (res['status'] == 201 || res['status'] == 200) {
      ...
      if (eventData['voiceEnabled'] == true) {
        await _speak(
            'Event ${eventData['title']} created. It starts at ${DateFormat('hh:mm a').format(startTime)}');
      }
      await _loadEvents();
      if (mounted)
  ...
  Future<void> _createReminderForSelected(Map<String, dynamic> remData) async {
    ...
    if (res['status'] == 201 || res['status'] == 200) {
      ...
      if ((remData['notificationEnabled'] ?? true) &&
        ...
        await _speak(
            'Reminder set. ${payload['title']} at ${DateFormat('hh:mm a').format(scheduled)}');
      }
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    final hasAnything = eventsForSelected.isNotEmpty ||
        remindersForSelected.isNotEmpty ||
        habitsForSelected.isNotEmpty ||
        routinesForSelected.isNotEmpty;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_isSpeaking)
            IconButton(
                icon: const Icon(Icons.volume_off, color: Color(0xFFFF6A00)),
                onPressed: () async {
                  await _tts.stop();
                  setState(() => _isSpeaking = false);
                }),
      ...
      body: _loading
          ...
          : Column(children: [
                ...
                child: TableCalendar(
                  ...
                  calendarBuilders: CalendarBuilders(
                    markerBuilder: (context, day, events) {
                      ...
                      final hasHabit =
                          (_habitMarkers[_dateOnly(day)] ?? []).isNotEmpty;
                      final hasRoutine =
                          (_routineMarkers[_dateOnly(day)] ?? []).isNotEmpty;
                      final hasEvent =
                          (_events[_dateOnly(day)] ?? []).isNotEmpty;
                      return Positioned(
                          bottom: 1,
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            if (hasEvent) _dot(const Color(0xFFFF6A00)),
                  ...
                  calendarStyle: const CalendarStyle(
                    ...
                    todayDecoration: BoxDecoration(
                        color: Color(0x44FF6A00), shape: BoxShape.circle),
              ...
              Padding(
                  ...
                  child: Row(children: [
                    _legendItem(const Color(0xFFFF6A00), 'Events'),
                    ...
                    : ListView(
                        ...
                        children: [
                          ...
                          if (eventsForSelected.isNotEmpty) ...[
                            _sectionHeader(
                                Icons.event, 'Events', const Color(0xFFFF6A00)),
                            const SizedBox(height: 8),
                                      ...
                                      child: ListTile(
                                        ...
                                        trailing: IconButton(
                                            icon: Icon(
                                                ...
                                                size: 18),
                                            onPressed: () => _speak(
                                                'Event $title from ${DateFormat('hh:mm a').format(start)} to ${DateFormat('hh:mm a').format(end)}. ${loc.isNotEmpty ? 'At $loc.' : ''} ${desc.isNotEmpty ? desc : ''}')),
                                      ),
                                    ),
                                  ));
                            }).toList(),
                          ...
                          if (remindersForSelected.isNotEmpty) ...[
                            ...
                            _sectionHeader(
                                Icons.notifications, 'Reminders', Colors.amber),
                            const SizedBox(height: 8),
                                      ...
                                      child: ListTile(
                                        ...
                                        trailing: IconButton(
                                            icon: Icon(
                                                ...
                                                size: 18),
                                            onPressed: () => _speak(
                                                'Reminder: $title at ${DateFormat('hh:mm a').format(time)}. ${desc.isNotEmpty ? desc : ''}')),
                                      ),
                                    ),
                                  ));
                            }).toList(),
                          ...
                          if (habitsForSelected.isNotEmpty) ...[
                            const SizedBox(height: 10),
                            _sectionHeader(
                                Icons.track_changes, 'Habits', Colors.orange),
                            const SizedBox(height: 8),
                            ...habitsForSelected
                                .map((h) => Padding(
                                      padding: const EdgeInsets.only(bottom: 8),
                                        ...
                                        child: Row(children: [
                                          ...
                                          IconButton(
                                              icon: Icon(
                                                  ...
                                                  size: 18),
                                              onPressed: () => _speak(
                                                  'Habit: ${(h['title'] ?? '').replaceAll('🎯', '')}. ${(h['streak'] ?? 0) > 0 ? 'Current streak ${h['streak']} days.' : ''}')),
                                        ]),
                                      ),
                                    ))
                                .toList(),
                          ...
                          if (routinesForSelected.isNotEmpty) ...[
                            const SizedBox(height: 10),
                            _sectionHeader(
                                Icons.auto_awesome, 'Routines', Colors.purple),
                            const SizedBox(height: 8),
                                    ...
                                    IconButton(
                                        ...
                                        onPressed: () {
                                          final name = r['title']
                                              ...
                                              '';
                                          _speak(
                                              '$label: $name at ${DateFormat('hh:mm a').format(r['time'] as DateTime)}${duration != null ? ". Duration $duration minutes" : ""}');
                                        }),
                                  ]),
                                ),
                              );
      ...
      builder: (BuildContext ctx) => StatefulBuilder(
          builder: (ctx, ss) => AlertDialog(
                ...
                title: const Row(children: [
                  Icon(Icons.event_note, color: Color(0xFFFF6A00)),
                  SizedBox(width: 8),
                ...
                content: SizedBox(
                    ...
                    child: SingleChildScrollView(
                        child: Column(
                          ...
                          const SizedBox(height: 10),
                          _df(descCtl, 'Description', Icons.description,
                              maxLines: 2),
                          const SizedBox(height: 10),
                          ...
                          _dd(
                              ...
                              onChanged: (v) =>
                                  ss(() => reminderMinutes = int.parse(v!))),
                          const SizedBox(height: 16),
                          ...
                          if (voiceEnabled) ...[
                            ...
                            const SizedBox(height: 10),
                            _df(voiceMsgCtl, 'Custom Voice Message (optional)',
                                Icons.mic),
                          ],
                          const SizedBox(height: 4),
                        ]))),
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () {
                      ...
                      Navigator.of(ctx).pop({
                        ...
                        'voiceCustomMessage': voiceMsgCtl.text.trim(),
  ...
  // ── Shared helpers ────────────────────────────────────────────────────────
  Widget _df(TextEditingController ctl, String hint, IconData icon,
          {int maxLines = 1}) =>
      TextField(
          controller: ctl,
          maxLines: maxLines,
  ...
  Widget _tr(BuildContext ctx, String label, IconData icon, int h, int m,
          ...
          onTap: () async {
            final t = await showTimePicker(
                ...
                builder: (c, ch) => Theme(
                    data: Theme.of(c).copyWith(
                        colorScheme:
                            const ColorScheme.dark(primary: Color(0xFFFF6A00))),
                    child: ch!));
  ...
  Widget _buildSH(String title) => Text(title,
      style: const TextStyle(
          color: Color(0xFFFF6A00), fontWeight: FontWeight.w600, fontSize: 15));
