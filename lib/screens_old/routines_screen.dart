...
Future<void> _scheduleDailyNotif({
  ...
  try {
    ...
    debugPrint('🔔 Scheduled daily notif: $title at $hour:$minute');
  } catch (e) {
    debugPrint('⚠️ Schedule notif error: $e');
  }
}

Future<void> _cancelRoutineNotifs(String routineId) async {
  await _initNotifs();
  // Cancel range of IDs used for this routine
  final hash = routineId.hashCode.abs() % 10000;
  for (int i = 0; i < 50; i++) {
    await _notifs.cancel(hash + i);
  }
}
...
class _RoutinesScreenState extends State<RoutinesScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      debugPrint('Load routines: $e');
    }
    setState(() => _loading = false);
  }

  Future<void> _completeMorning(String id) async {
    final res = await http.post(Uri.parse('$_base/$id/complete-morning'),
        headers: _headers);
    if (!mounted) return;
    final body = jsonDecode(res.body);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(res.statusCode == 200
            ...
            : (body['message'] ?? 'Failed')),
        backgroundColor:
            res.statusCode == 200 ? Colors.green[800] : Colors.orange[800]));
    if (res.statusCode == 200) _load();
  }

  Future<void> _completeEvening(String id) async {
    final res = await http.post(Uri.parse('$_base/$id/complete-evening'),
        headers: _headers);
    if (!mounted) return;
    final body = jsonDecode(res.body);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(res.statusCode == 200
            ...
            : (body['message'] ?? 'Failed')),
        backgroundColor:
            res.statusCode == 200 ? Colors.purple[800] : Colors.orange[800]));
    if (res.statusCode == 200) _load();
  }
  ...
  void _showForm({Map<String, dynamic>? existing}) {
    showModalBottomSheet(
      ...
      builder: (_) =>
          _RoutineFormSheet(token: _token, existing: existing, onSaved: _load),
    );
  }
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return _RoutineCard(
                        ...
                        onDelete: () =>
                            _delete(r['_id'] ?? r['id'] ?? '', r['name'] ?? ''),
                        onMorning: () =>
                            _completeMorning(r['_id'] ?? r['id'] ?? ''),
                        onEvening: () =>
                            _completeEvening(r['_id'] ?? r['id'] ?? ''),
                      );
                    },
                  )),
    );
...
class _RoutineCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final morningEnabled =
        (routine['morningRoutine'] as Map?)?['enabled'] == true;
    final eveningEnabled =
        (routine['eveningRoutine'] as Map?)?['enabled'] == true;
    final wakeH =
        ((routine['morningRoutine'] as Map?)?['wakeUpTime'] as Map?)?['hour'] ??
            6;
    final wakeM = ((routine['morningRoutine'] as Map?)?['wakeUpTime']
            as Map?)?['minute'] ??
        0;
    final sleepH =
        ((routine['eveningRoutine'] as Map?)?['sleepTime'] as Map?)?['hour'] ??
            22;
    final sleepM = ((routine['eveningRoutine'] as Map?)?['sleepTime']
            as Map?)?['minute'] ??
        0;
    final morningActs =
        ((routine['morningRoutine'] as Map?)?['activities'] as List?) ?? [];
    final eveningActs =
        ((routine['eveningRoutine'] as Map?)?['activities'] as List?) ?? [];
        ...
        Padding(
            ...
            child: Row(children: [
                  ...
                  child: Column(
                    ...
                    if (days.isNotEmpty)
                      Text(
                          days
                              .map((d) =>
                                  d.toString().substring(0, 2).toUpperCase())
                              .join(' · '),
                          style:
                              TextStyle(color: Colors.grey[500], fontSize: 11)),
                    if (endDate != null)
                      Text('Until ${endDate.substring(0, 10)}',
                          style: TextStyle(
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Row(children: [
                  _statBadge(
                      '🏆 ${stats['currentStreak'] ?? 0}', Colors.orange),
                  const SizedBox(width: 4),
                  _statBadge(
                      '🌅 ${stats['morningCompletions'] ?? 0}', Colors.blue),
                  const SizedBox(width: 4),
                  _statBadge(
                      '🌆 ${stats['eveningCompletions'] ?? 0}', Colors.purple),
                ]),
              ]),
                  ...
                  children: [
                    ...
                    if (morningActs.isNotEmpty) ...[
                      ...
                      if (morningActs.length > 4)
                        Text('+${morningActs.length - 4} more',
                            style: TextStyle(
                  ...
                  children: [
                    ...
                    if (eveningActs.isNotEmpty) ...[
                      ...
                      if (eveningActs.length > 4)
                        Text('+${eveningActs.length - 4} more',
                            style: TextStyle(
                                color: Colors.grey[600], fontSize: 11)),
                    ],
                    const SizedBox(height: 10),
...
class _RoutineFormSheetState extends State<_RoutineFormSheet> {
  ...
  void initState() {
    ...
    if (widget.existing != null) {
      ...
      _days =
          ((r['schedule'] as Map?)?['days'] as List?)?.cast<String>() ?? _days;
      final period = r['period'] as Map? ?? {};
      if (period['startDate'] != null) {
        _startDate = DateTime.tryParse(period['startDate']) ?? DateTime.now();
      }
      if (period['endDate'] != null) {
        _endDate = DateTime.tryParse(period['endDate']);
      }
      ...
      _morningActs = List<Map<String, dynamic>>.from(
          ((m['activities'] as List?) ?? [])
      ...
      _eveningActs = List<Map<String, dynamic>>.from(
          ((e['activities'] as List?) ?? [])
              .map((a) => Map<String, dynamic>.from(a as Map)));
    }
  }
  ...
  Future<void> _pickTime(
      String label, int h, int m, void Function(int, int) onPicked) async {
  ...
  Future<void> _pickDate({required bool isEnd}) async {
    ...
    final p = await showDatePicker(
      context: context,
      initialDate: initial,
      firstDate: isEnd ? _startDate : DateTime(2024),
      lastDate: DateTime(2030),
      helpText: isEnd ? 'Routine End Date' : 'Routine Start Date',
      builder: (ctx, child) => Theme(
          data: Theme.of(ctx).copyWith(
              colorScheme: const ColorScheme.dark(primary: Color(0xFFFF6A00))),
          child: child!),
  ...
  // Schedule all local notifications for this routine
  Future<void> _scheduleNotifications(String routineId) async {
    ...
    if (_morningEnabled) {
      await _scheduleDailyNotif(
        ...
        title: '🌅 Wake Up Time!',
        body:
            '${_nameCtl.text.trim()} — Good morning! Time to start your day at ${_fmt(_wakeH, _wakeM)}',
        hour: _wakeH,
        minute: _wakeM,
        days: _days,
      );

      // Morning activity notifications
      for (final act in _morningActs) {
        ...
        if (title.isNotEmpty) {
          await _scheduleDailyNotif(
            ...
            title: '⚡ $title',
            body:
                'Morning activity — ${act['duration'] ?? 30} minutes | ${_nameCtl.text.trim()}',
            hour: aH,
            minute: aM,
            days: _days,
          );
    ...
    // Sleep notification (30 min before sleep time)
    if (_eveningEnabled) {
      ...
      await _scheduleDailyNotif(
        ...
        title: '🌙 Bedtime Soon',
        body:
            '${_nameCtl.text.trim()} — Wind down! Sleep time in 30 minutes at ${_fmt(_sleepH, _sleepM)}',
        hour: beforeH.clamp(0, 23),
        minute: beforeM.clamp(0, 59),
        days: _days,
      );
      ...
      await _scheduleDailyNotif(
        ...
        body:
            '${_nameCtl.text.trim()} — Time for your evening routine! Sleep at ${_fmt(_sleepH, _sleepM)}',
        hour: _sleepH,
        minute: _sleepM,
        days: _days,
      );

      // Evening activity notifications
      for (final act in _eveningActs) {
        ...
        if (title.isNotEmpty) {
          await _scheduleDailyNotif(
            ...
            title: '🌆 $title',
            body:
                'Evening activity — ${act['duration'] ?? 30} minutes | ${_nameCtl.text.trim()}',
            hour: aH,
            minute: aM,
            days: _days,
          );
    ...
    debugPrint('✅ Scheduled ${notifId - hash} notifications for routine');
  }
  ...
  Future<void> _save() async {
    ...
    final payload = {
      ...
      'morningRoutine': {
        ...
        'activities': _morningActs
            .map((a) => {
                  'title': a['title'] ?? '',
                  'startTime':
                      a['startTime'] ?? {'hour': _wakeH, 'minute': _wakeM},
      ...
      'eveningRoutine': {
        ...
        'activities': _eveningActs
            .map((a) => {
                  'title': a['title'] ?? '',
                  'startTime':
                      a['startTime'] ?? {'hour': _sleepH, 'minute': _sleepM},
                  'duration': a['duration'] ?? 30,
                  'priority': a['priority'] ?? 'medium',
                  'category': a['category'] ?? 'personal',
                  'addToCalendar': true,
    ...
    try {
      ...
      final id =
          isEdit ? (widget.existing!['_id'] ?? widget.existing!['id']) : null;
      ...
      if (res.statusCode == 200 || res.statusCode == 201) {
        ...
        final savedId = responseBody['data']?['_id'] ??
            responseBody['routine']?['_id'] ??
            id ??
            '';

        // Cancel old notifs and schedule new ones
        if (savedId.isNotEmpty) {
          await _cancelRoutineNotifs(savedId);
          await _scheduleNotifications(savedId);
        }
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(isEdit
                ? '✅ Routine updated! Notifications scheduled.'
                : '✅ Routine created! Notifications scheduled.'),
  ...
  void _addAct(bool isMorning) => setState(() {
        ...
        if (isMorning) {
          _morningActs.add({
            'title': '',
            'startTime': {'hour': defaultH, 'minute': defaultM},
            'duration': 30,
            'priority': 'medium',
            'category': 'personal'
          });
  ...
  void _removeAct(bool isMorning, int idx) => setState(() {
        if (isMorning)
          _morningActs.removeAt(idx);
        else
          _eveningActs.removeAt(idx);
      });
        ...
        child: Column(children: [
          ...
          Expanded(
            ...
            child:
              ...
              // ── Morning ───────────────────────────────────────────────────
              _sectionHeader('🌅 Morning Routine', _morningEnabled,
                  (v) => setState(() => _morningEnabled = v), Colors.orange),
              if (_morningEnabled) ...[
                const SizedBox(height: 10),
                GestureDetector(
                  onTap: () =>
                      _pickTime('Wake Up Time', _wakeH, _wakeM, (h, m) {
                    _wakeH = h;
                    _wakeM = m;
                  }),
                  child: _timeDisplay(
                      '⏰ Wake Up Time', _fmt(_wakeH, _wakeM), Colors.orange,
                      subtitle:
                          'Tap to change • 🔔 Notification will be scheduled'),
                ),
                const SizedBox(height: 10),
                ..._morningActs
                    .asMap()
                    .entries
                    .map((e) => _actTile(e.value, e.key, true)),
                TextButton.icon(
                    onPressed: () => _addAct(true),
                    icon: const Icon(Icons.add,
                        color: Color(0xFFFF6A00), size: 18),
                    label: const Text('Add Morning Activity',
                        style: TextStyle(color: Color(0xFFFF6A00)))),
              ],
              const SizedBox(height: 8),

              // ── Evening ───────────────────────────────────────────────────
              _sectionHeader('🌆 Evening Routine', _eveningEnabled,
                  (v) => setState(() => _eveningEnabled = v), Colors.purple),
              if (_eveningEnabled) ...[
                const SizedBox(height: 10),
                GestureDetector(
                  onTap: () =>
                      _pickTime('Sleep Time', _sleepH, _sleepM, (h, m) {
                    _sleepH = h;
                    _sleepM = m;
                  }),
                  child: _timeDisplay(
                      '🌙 Sleep Time', _fmt(_sleepH, _sleepM), Colors.purple,
                      subtitle: 'Tap to change • 🔔 Reminder 30 min before'),
                ),
                const SizedBox(height: 10),
                ..._eveningActs
                    .asMap()
                    .entries
                    .map((e) => _actTile(e.value, e.key, false)),
                TextButton.icon(
                    onPressed: () => _addAct(false),
                    icon: const Icon(Icons.add,
                        color: Color(0xFFFF6A00), size: 18),
                    label: const Text('Add Evening Activity',
                        style: TextStyle(color: Color(0xFFFF6A00)))),
              ],
  ...
  Widget _timeDisplay(String label, String time, Color color,
          {String? subtitle}) =>
      Container(
  ...
  Widget _actTile(Map<String, dynamic> act, int idx, bool isMorning) {
    ...
    final aH =
        (act['startTime'] as Map?)?['hour'] ?? (isMorning ? _wakeH : _sleepH);
    final aM = (act['startTime'] as Map?)?['minute'] ?? 0;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(
              child: TextField(
            controller: titleCtl,
            onChanged: (v) => act['title'] = v,
            style: const TextStyle(color: Colors.white, fontSize: 13),
            decoration: InputDecoration(
                hintText: 'Activity name (e.g. Meditation)',
                hintStyle: TextStyle(color: Colors.grey[600], fontSize: 12),
                border: InputBorder.none,
                contentPadding: EdgeInsets.zero),
          )),
          GestureDetector(
              onTap: () => _removeAct(isMorning, idx),
        ...
        Row(children: [
          ...
          Expanded(
              child: GestureDetector(
            onTap: () => _pickTime('Activity Start Time', aH, aM, (h, m) {
              setState(() {
                act['startTime'] = {'hour': h, 'minute': m};
              });
            }),
            child: Container(
  ...
  Widget _sectionHeader(String title, bool enabled,
          void Function(bool) onToggle, Color color) =>
      Container(
