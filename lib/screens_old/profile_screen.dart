...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadManualData() async {
    ...
    final service = HealthService(auth: auth);
    final data = await service.getManualData();
      ...
      if (res.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          setState(() {
            ...
            _stepGoalCtl.text =
                (user['healthGoals']?['dailySteps'] ?? 10000).toString();
            _sleepGoalCtl.text =
                (user['healthGoals']?['sleepHours'] ?? 8).toString();
    ...
    try {
      await http
          .put(
            ...
            body: jsonEncode({'name': _nameCtl.text.trim()}),
          )
          .timeout(const Duration(seconds: 10));
  ...
  void dispose() {
    _nameCtl.dispose();
    _stepGoalCtl.dispose();
    _sleepGoalCtl.dispose();
    super.dispose();
  }
              ...
              children: [
                ...
                _buildSection('Health Goals', [
                  _inputTile(
                      ...
                      controller: _stepGoalCtl,
                      icon: Icons.directions_walk,
                      keyboardType: TextInputType.number),
                  _inputTile(
                      label: 'Sleep Goal (hours)',
                      controller: _sleepGoalCtl,
                      icon: Icons.nightlight_round,
                      keyboardType:
                          const TextInputType.numberWithOptions(decimal: true)),
                ]),
                const SizedBox(height: 20),
                _buildSection('Doxy Preferences', [
                ...
                const SizedBox(height: 20),
                if (_manualData.isNotEmpty) _buildManualHealthSection(),
                if (_manualData.isNotEmpty) const SizedBox(height: 20),
                _buildAccountInfo(),
              ],
            ),
    );
  ...
  Widget _buildManualHealthSection() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        child: Column(children: [
          Row(children: [
            const Icon(Icons.edit_note, color: Color(0xFFFF6A00), size: 16),
            const SizedBox(width: 6),
            Text('Manually entered today',
                style: TextStyle(color: Colors.grey[500], fontSize: 12)),
          ]),
          const SizedBox(height: 10),
          if (_manualData['heartRate'] != null)
            _manualRow('❤️ Heart Rate', '${_manualData['heartRate']} bpm',
                Colors.pinkAccent),
          if (_manualData['sleepHours'] != null)
            _manualRow('😴 Sleep', '${_manualData['sleepHours']} hours',
                Colors.purpleAccent),
          if (_manualData['weight'] != null)
            _manualRow('⚖️ Weight', '${_manualData['weight']} kg',
                Colors.lightBlueAccent),
          const SizedBox(height: 8),
          Text('Go to Health tab to update these values',
  ...
  Widget _buildAccountInfo() {
    ...
    if (joined != null) {
      try {
        final dt = DateTime.parse(joined);
